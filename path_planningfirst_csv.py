...
def read_setpoints_from_csv(file_path):
    setpoints = [[0, 0, 0.4]]  # Starting point

    # Read the CSV and collect gate coordinates
    gates = []
    ...
    setpoints.extend(gates)      # First pass through gates
    setpoints.extend(gates)      # Second pass through same gates
    setpoints.append([0, 0, 0.8]) # Final point

    return setpoints

def read_setpoints_from_csv_entry_exit(file_path, offset=0.5):
    setpoints = [[0, 0, 0.4]]  # Starting point
    ...
    with open(file_path, newline='') as csvfile:
        ...
        for row in reader:
            ...
            y = float(row['y'])
            z = float(row['z'])
            theta = float(row['theta'])  # Read the theta value



            # Calculate entry and exit points based on theta
            entry_x = x - offset * np.sin(theta)
            entry_y = y - offset * np.cos(theta)
            exit_x = x + offset * np.sin(theta)
            exit_y = y + offset * np.cos(theta)
            ...
            # Append entry, gate center, and exit points
            gates.append([entry_x, entry_y, z])  # Entry point
            # gates.append([x, y, z])             # Gate center
            gates.append([exit_x, exit_y, z])   # Exit point
...
def adaptive_times(setpoints, total_time=15):
    """
    Calculate adaptive time points based on distances between setpoints.

    Parameters:
        setpoints (list of list): 3D coordinates of the setpoints.
        total_time (float): Total duration for the trajectory.

    Returns:
        np.ndarray: Array of adaptive time points.
    """
    # Calculate distances between consecutive setpoints
    distances = [np.linalg.norm(np.array(setpoints[i+1]) - np.array(setpoints[i])) 
    ...
    # Calculate proportional time intervals
    proportional_times = distances / total_distance * total_time

    # Cumulative sum to get actual time points, starting from 0
    times = np.insert(np.cumsum(proportional_times), 0, 0)
...
if __name__ == "__main__":
    ...
    # setpoints = [[0, 0, 0.4], [0.89, -0.17, 0.77], [1.8, 0.27, 1.2], [1.8, 0.27, 1.2], [-0.21, 0.65, 1.68]]
    # setpoints = [[0, 0, 0.4], [0.82, -0.14, 0.82], [1.82, 0.35, 1.18], [0.80, 0.82, 1.46], [-0.06, 0.66, 1.61], [0, 0, 0.8], [0,0,-0.1]]
    # setpoints = [[0, 0, 0.4], [0.56, -.28, 0.84], [1.98, 0.34, 1.21], [0.83, 1.17, 1.50], [-0.13, 0.51, 1.64], [0.56, -.28, 0.84], [1.98, 0.34, 1.21], [0.83, 1.17, 1.50], [-0.13, 0.51, 1.64], [0, 0, 0.8]]
    if ENTRY_EXIT:
        setpoints = read_setpoints_from_csv_entry_exit("gates_info.csv", offset=0.2)
    else:
        setpoints = read_setpoints_from_csv("gates_info.csv")

    print("Setpoints:", setpoints)

    # times = np.linspace(0, 25, len(setpoints)) #15 for one lap, 20 for two laps
    times = adaptive_times(setpoints, total_time=15) # 15 is safe
    yaw_setpoints = [0, 0, 90, -180, -90, 0]
    disc_steps = 5
    ...
    time_setpoint_diff = np.diff(time_setpoints)

    # trajectory_setpoints_test = [[0, 0, 0.4, 0], [0, 0, 0.8, 0], [0.8, -0.5, 0.8, 0], ]
    ...
    while le.is_connected:
        ...
        print("Moving to gates")
        # Move
        for setpoint, time_setpoint in zip(trajectory_setpoints, time_setpoint_diff):
            print("Setpoint:", setpoint)
            print("Time setpoint:", time_setpoint)
        ...
        # Land
        for _ in range(20):
            cf.commander.send_position_setpoint(0, 0, 0.8, 0)
            time.sleep(0.1)
        for _ in range(10):
